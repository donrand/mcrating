...
export default async function BattlesPage() {
  const { data: battles } = await supabase
    .from('battles')
    .select('id, winner, round_name, approved_at, tournaments(name, held_on), mc_a:mcs!battles_mc_a_id_fkey(id, name), mc_b:mcs!battles_mc_b_id_fkey(id, name), ratings(mc_id, delta, rating_after)')
    .eq('status', 'approved')
